\section{Results}

\subsection{Traditional Predictive Models}
\begin{figure}
	\includegraphics[width=\textwidth]{code/plots/model_performance.png}
	\caption{Performance comparison of traditional predictive models}
\end{figure}
Various traditional model architectures are implemented. Of particular are Random Forest, Decision Tree, Gradient Boosting, Support Vector Machine, Gaussian Naive Bayes and K-Nearest Neighbors. The performances of each model is measured using plain accuracy, balanced accuracy, and F1 score. The accuracy of a model can be calculated as follows:
\[
	\text{Accuracy} = \frac{\text{True Positive} + \text{True Negative}}{\text{Total Prediction}}.
\]
It is an intuitive measure of how correct the model predictions are, but falls short when the given dataset is imbalanced. To mitigate this issue, the balanced accuracy is preferred:
\[
	\text{Balanced Accuracy} = \frac{\text{Sensitivity} + \text{Specificity}}{2}.
\]
In addition, the F1 score is also utilized, calculated as the harmonic mean of precision and recall:
\[
	\text{F1 Score} = \frac{2 \times \text{Precision} \times \text{Recall}}{\text{Precision} + \text{Recall}}.
\]
As shown in Fig. 1, Gradient Boosting consistently outperforms other predictive models.

\subsection{Large Language Models}
\begin{figure}
	\begin{minipage}{0.5\textwidth}
		\includegraphics[width=\textwidth]{code/plots/qwen.png}
	\end{minipage}
	\begin{minipage}{0.5\textwidth}
		\includegraphics[width=\textwidth]{code/plots/llama.png}
	\end{minipage}
	\caption{Performance comparison of Qwen2.5-3B and Meta-Llama3-9B}
\end{figure}
We use a modified metric for LLMs---Macro F1. This metric effectively treats each class in a model equally important by taking the mean of the F1 scores of each class. Observe that it performs noticeably worse than the lowest scoring traditional predictive model.